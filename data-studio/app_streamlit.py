...
def piechart_plot(dataframe):
    ...
    for i in columns:
        if dataframe[i].dtypes not in numeric and dataframe[i].dtypes in non_numeric:
            non_num_columns.append(i)
            columns.remove(i)

    column1 = st.selectbox("Column 1(categorical)", np.concatenate([['Select'], non_num_columns]))
...
def test(dataframe):
    ...
    for i in columns:
        if dataframe[i].dtypes not in numeric:
            columns.remove(i)
